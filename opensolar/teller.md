...
The price for energy per kWH is derived from standard government sources and articles. For the pilot platform, this price is fixed at $0.20/kWh.

The teller also performs auxiliary functions that are required to prove ownership and correct accumulation of data. It tracks the device's location, stores the solar grid's state on IPFS every week, stores the associated IPFS hash on the blockchain at regular intervals, and provides functions for a receiver to view the balance and other data associated with a project. These functions can be accessed through the teller's Command line interface.

The teller requires two things apart from the source itself to start:

1. A config file: The teller needs a config file in order to be able to work correctly. This config file is based on the `dummyconfig.yaml` file in the repository and contains detials of the recipient, their password, mqtt credentials, etc.
2. An SSL certificate file \(self signed / thir party signed\): This is required in order for the teller to be able to listen on port 443 and serve pieces of data when required/
